...
rename_original_pvcs() {
    ...
    done < <(jq -c '.[] | select(.original_rename_complete | not)' "$pvc_backup_file")
...
rename_migration_pvcs() {
    ...
    original_pvc_count=$(jq '[.[] | select(.matched | not)] | length' "$original_app_pvc_info")
    new_pvc_count=$(jq '[.[] | select(.matched | not)] | length' "$new_app_pvc_info")
