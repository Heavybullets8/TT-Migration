...
match_pvcs_with_mountpoints() {
    ...
    original_pvcs=$(jq -c '.[] | select(.matched | not)' "$original_pvc_info_file")
    new_pvcs=$(jq -c '.[] | select(.matched | not)' "$new_pvc_info_file")
...
match_remaining_single_pvc_pair() {
    ...
    local original_pvc=$(jq -c '.[] | select(.matched | not)' "$original_app_pvc_info")
    local new_pvc=$(jq -c '.[] | select(.matched | not)' "$new_app_pvc_info")
...
match_remaining_pvcs_by_name() {
    ...
    local original_pvcs=$(jq -c '.[] | select(.matched | not)' "$original_app_pvc_info")
    local new_pvcs=$(jq -c '.[] | select(.matched | not)' "$new_app_pvc_info")
